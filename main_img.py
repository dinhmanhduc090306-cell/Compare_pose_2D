...
def step(split, opt, actions, dataLoader, model, optimizer=None, epoch=None, writer=None, adaptive_weight=None):
    ...
    for i, data in TQDM:
        ...
        if split == 'train':
            ...
            # --- ADD THIS BLOCK TO SAVE YOUR 3D POSES ---
            import numpy as np
            # Save the raw 3D predictions (Batch, Frames, 17 joints, 3 coordinates)
            # You can dynamically name this based on the subject or action if preferred
            np.save(f'output/predicted_poses_{subject[0]}_{action[0]}.npy', output_3D.cpu().numpy())
            # --------------------------------------------
        ...
        if split == 'train':
            ...
            action_error_sum = test_calculation(output_3D, out_target, action, action_error_sum, opt.dataset, subject, valid_mask=valid_mask)
...
if __name__ == '__main__':
    ...
    if opt.dataset == 'ap3d':
        ...
        from evaluate_ap3d import define_ap3d_actions
